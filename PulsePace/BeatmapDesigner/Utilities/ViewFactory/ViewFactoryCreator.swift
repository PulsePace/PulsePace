...
class ViewFactoryCreator {
    ...
    func createCanvasView(
        for object: any HitObject,
        at cursorTime: Double,
        with spacing: CGFloat
    ) -> some View {
        if let tapHitObject = object as? TapHitObject {
            return AnyView(TapHitObjectViewFactory().createCanvasView(
                for: tapHitObject,
                at: cursorTime,
                with: spacing
            ))
        } else if let slideHitObject = object as? SlideHitObject {
            return AnyView(SlideHitObjectViewFactory().createCanvasView(
                for: slideHitObject,
                at: cursorTime,
                with: spacing
            ))
        } else if let holdHitObject = object as? HoldHitObject {
            return AnyView(HoldHitObjectViewFactory().createCanvasView(
                for: holdHitObject,
                at: cursorTime,
                with: spacing
            ))
